    ...
    , _enforceSendingSelfID (false)
{
    ...
    // "Fixed" operator location is no longer user-selectable; ensure we always
    // start in LiveGNSS mode even if a previous app version stored FIXED.
    if (_settings->locationType()->rawValue().toUInt() != LocationTypes::LiveGNSS) {
        _settings->locationType()->setRawValue(static_cast<uint>(LocationTypes::LiveGNSS));
    }

    // Track construction time for the initial-GCS-wait timer in _sendSystem.
    _startupTime = QDateTime::currentDateTime().currentDateTimeUtc();

    // RID is unconditionally force-enabled on connect (no longer gated by a
    // user-facing setting; the toggle was removed from GeneralSettings.qml).
    _available = true;
    _commsGood = true;
    _checkGCSBasicID();
...
{
    // Operator-location source selection. The FAA / OpenDroneID protocol
    // expects the operator's position. We prefer the Android GCS GPS, but
    // it can take a long time to acquire. For test purposes we allow falling
    // back to the drone's position when the GCS hasn't produced a fix.
    //
    // States (driven each tick):
    //   1. GCS currently has a fresh, valid fix -> send GCS position.
    //   2. Drone-fallback active (we previously decided to use the drone)
    //      -> send drone coordinate (if valid); otherwise unknown.
    //   3. GCS has never been good yet -> wait. If startup-wait expired and
    //      the drone has a valid position, switch to drone-fallback.
    //   4. GCS was good before but is currently stale -> keep sending the
    //      last-known good GCS position. While the drone is disarmed, run a
    //      re-eligibility timer; when it elapses without GCS recovery,
    //      switch to drone-fallback. While armed, the timer is reset.

    const QDateTime now = QDateTime::currentDateTime().currentDateTimeUtc();
    const QGeoCoordinate currentGcsPosition = QGCPositionManager::instance()->gcsPosition();
    const QGeoPositionInfo currentGeoInfo   = QGCPositionManager::instance()->geoPositionInfo();
    ...
    QGeoCoordinate vehicleCoord = _vehicle->coordinate();
    ...
    if (gcsCurrentlyGood) {
        ...
        // State 2: latched into drone fallback. The drone position is meant
        // to *approximate* the operator's location on the ground — so we
        // only refresh from the drone while it's disarmed. Once armed, the
        // last broadcast value is frozen so the operator location doesn't
        // track the drone's flight path.
        ...
        // drone — but only if it's still on the ground (disarmed). Capturing
        // a mid-flight drone position as the operator location would be
        // misleading, so we'd rather keep waiting in that case.
        ...
        // Re-eligibility for drone-fallback: only while the drone is disarmed.
        if (!_vehicle->armed()) {
    ...
    // Track the broadcast position + source so the QML settings page can
    // display whatever's actually going out on the wire (GCS or drone).
    ...
    if (sharedLink) {
        ...
        mavlink_msg_open_drone_id_system_pack_chan(MAVLinkProtocol::instance()->getSystemId(),
                                                    ...
                                                    positionValid ? ( positionToSend.latitude()  * 1.0e7 ) : MAVLINK_UNKNOWN_LAT,
                                                    positionValid ? ( positionToSend.longitude() * 1.0e7 ) : MAVLINK_UNKNOWN_LON,
                                                    AREA_COUNT,
                                                    AREA_RADIUS,
                                                    ...
                                                    positionValid ? positionToSend.altitude() : MAVLINK_UNKNOWN_METERS,
